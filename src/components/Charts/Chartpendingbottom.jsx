...
const HorizontalBarChartBottom = ({ bottom5 }) => {
  ...
  const labels = bottom5
    ? bottom5.map((item, index) => String.fromCharCode(65 + index))
    : []; // Generate labels A, B, C, D, E

  const lablesDisplay = bottom5 ? bottom5.map((item) => item.CustName) : [];
  const data = bottom5 ? bottom5.map((item) => Math.abs(item.Amount)) : [];
...
export default HorizontalBarChartBottom;
